...
library(ggplot2)
g<-ggplot(data=per.var, aes(x=factor(PC), y=PercentVariance, axis.ticks=1)) + geom_bar(stat="identity") + xlab("principal component") + ylab("% Variance") + theme(text=element_text(size=14)) + ggtitle("Variance Explained by PCs")
print(g)
# write graph out 
...
## Plot PC1 vs. PC2
g<-autoplot(df.pca, data=df, colour='Species', alpha=0.5, variance_percentage=TRUE, loadings=FALSE)
print(g)
```
2. Plot PC2 vs. PC 3
```{r plot_pca2, echo=TRUE, warnings=F}
## Plot PC2 vs. PC3
g<-autoplot(df.pca, data=df, colour='Species', alpha=0.5, variance_percentage=TRUE, loadings=FALSE, x=2, y=3)
print(g)
```
...
## Plot Densities of PCs 1 & 2 
```{r density, echo=TRUE, warnings=F}
## Density of PC1 Loadings plotted with base
d<-density(Loadings$PC1)
plot(d)

## Density of PC2 Loadings plotted with base
d2<-density(Loadings$PC2)
plot(d2)
```
